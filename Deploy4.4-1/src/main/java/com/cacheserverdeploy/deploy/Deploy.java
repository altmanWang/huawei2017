...
public class Deploy {
    ...
    public static String[] display(String[] graphContent) {
        ...
        ArrayList<Vertex<Integer>> candidates = opt.influenceMax();
        ...
        individual.initGeneSize(G.getNumberOfNodes());
        individual.gene[0] = true;
        individual.gene[3] = true;
        individual.gene[20] = true;
        individual.gene[22] = true;
        opt.getCost(individual);
        */

        String[] result = Optimization.getResult(best_individual);
        System.out.println("最終結果"+best_fitness_score);
...
class Optimization {
    ...
    */
    ...
    public void crossover(ArrayList<Chromosome> next_population) {
        ...
        ArrayList<Integer> list_b;
        Chromosome a_next;
        Chromosome b_next;
        while (next_population.size() < pop_size) {
            list_a = new ArrayList<Integer>();
            list_b = new ArrayList<Integer>();
            ...
            index_a = random.nextInt(list_a.size());
            index_b = random.nextInt(list_b.size());

            for(int i =0; i < list_a.size(); i++){
                if(i < index_a)
                    a_next.gene[list_a.get(i)] = true;
                else
                    b_next.gene[list_a.get(i)] = true;
            }

            for(int i =0; i < list_b.size(); i++){
                if(i < index_b)
                    b_next.gene[list_b.get(i)] = true;
                else
                    a_next.gene[list_b.get(i)] = true;
            }
