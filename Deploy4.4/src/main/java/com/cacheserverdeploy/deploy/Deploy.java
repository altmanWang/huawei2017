...
public class Deploy {
    ...
    public static String[] display(String[] graphContent) {
        ...
        opt.setup(candidates,graphContent);
        //opt.test();
        /*
        ...
        */
        ...
        /*
        int[] a = {7, 14, 18, 29, 35, 37, 38, 42, 49, 56, 58, 70, 79, 81, 88,
                94, 103, 110, 124, 126, 128, 146, 156, 161, 164, 167, 170, 174,
                190, 192, 197, 210, 213, 224, 225, 229, 239, 247, 249, 253, 260,
                262, 263, 266, 271, 281, 289, 298, 301, 308, 311, 312, 324, 325,
                340, 347, 363, 372, 376, 390, 391, 398, 404, 411, 413, 417, 419,
                420, 433, 440, 443, 444, 447, 449, 464, 467, 470, 487, 491, 497,
                506, 509, 512, 518, 523, 524, 538, 560, 561, 566, 568, 589, 605,
                610, 613, 624, 626, 630, 635, 637, 649, 655, 669, 681, 692, 698,
                705, 707, 710, 712, 714, 730, 736, 750, 754, 760, 774, 787, 790, 797};
        */
        //int[] a = {0,3,22};
        /*
        int[] a = {65, 131, 8, 74, 138, 78, 143, 80, 145, 147, 20, 149, 86,
                150, 26, 27, 91, 156, 29, 93, 157, 158, 32, 96, 97, 35, 36,
                100, 38, 102, 104, 42, 44, 109, 49, 113, 114, 51, 53, 117,
                54, 55, 122, 59, 127};
        */
        /*
        int[] a ={257, 131, 132, 5, 134, 7, 265, 267, 12,
                140, 15, 143, 271, 273, 18, 147, 148, 276,
                149, 150, 23, 279, 280, 153, 154, 282, 156,
                285, 30, 31, 160, 33, 162, 290, 165, 42, 298,
                45, 174, 176, 177, 178, 183, 57, 186, 59, 61,
                193, 66, 68, 69, 199, 200, 74, 206, 79, 207, 208,
                81, 82, 85, 86, 89, 222, 224, 97, 228, 230, 105,
                106, 107, 238, 112, 113, 244, 247, 249, 250
        };*/
        int[] a = {4, 8, 10, 12, 15, 25, 28, 29, 32, 39, 65, 68, 69,
                77, 78, 85, 87, 90, 91, 92, 95, 96, 103, 108, 109, 111, 117, 131, 133, 142, 144,
                146, 152, 158, 161, 167, 171, 176, 179, 180, 192, 196, 198, 210, 217, 218, 229,
                232, 236, 238, 241, 246, 250, 251, 264, 271, 284, 285, 291, 292
        };
        for(int i = 0; i < a.length; i++){
            individual.add(a[i]);
        }
        //ZKW.deployServer(graphContent);
        String[] result = opt.getResult(individual);
        return result;
    }
...
class Optimization {
    ...
    public void setup(ArrayList<Vertex<Integer>> candidates,String[] graphContent){
    ...
    //获得结果
    public String[] getResult(ArrayList<Integer> individual) {


        String[] result = ZKW.getWalk(graphContent,individual,weights_init,clients_init);
        return result;
    }
...
class ZKW {
    ...
    public static String[] deployServer(String[] graphContent) {
        ...
        for(int i=4;i<Integer.parseInt(nums[1])+4;i++){
            ...
            zkw.add(Integer.parseInt(data[0])+1, Integer.parseInt(data[1])+1, Integer.parseInt(data[2]), Integer.parseInt(data[3]));
            zkw.add(Integer.parseInt(data[1])+1, Integer.parseInt(data[0])+1, Integer.parseInt(data[2]), Integer.parseInt(data[3]));
        }
        for(int i=0;i<Integer.parseInt(nums[2]);i++){

            String[] data=graphContent[Integer.parseInt(nums[1])+5+i].split(" ");

            zkw.add(Integer.parseInt(data[0])+Integer.parseInt(nums[0])+1, Integer.parseInt(data[1])+1, Integer.parseInt(data[2]), 0);

            zkw.add(zkw.src, Integer.parseInt(data[0])+Integer.parseInt(nums[0])+1, Integer.parseInt(data[2]), 0);

        }

        int[] servers = {512, 260, 262, 518, 774, 7, 263, 266, 523, 524, 14,
                271, 18, 787, 790, 281, 538, 29, 797, 289, 35, 37, 38,
                42, 298, 301, 560, 49, 561, 308, 566, 311, 56, 312, 568,
                58, 324, 325, 70, 589, 79, 81, 340, 88, 347, 605, 94, 610,
                613, 103, 363, 110, 624, 626, 372, 630, 376, 635, 124, 637,
                126, 128, 390, 391, 649, 398, 655, 146, 404, 411, 156, 413,
                669, 161, 417, 419, 164, 420, 167, 681, 170, 174, 433, 692,
                440, 698, 443, 444, 190, 447, 192, 449, 705, 707, 197, 710,
                712, 714, 464, 210, 467, 213, 470, 730, 224, 736, 225, 229,
                487, 491, 750, 239, 497, 754, 247, 760, 249, 506, 253, 509};

        for(int i:servers){
            zkw.add(i+1,zkw.des, Integer.MAX_VALUE, 0);
        }


        long start=System.currentTimeMillis();
        zkw.costflow();
        System.out.println(zkw.ans);
    ...
    public static String[] getWalk(String[] graphContent,
        ...
        long start=System.currentTimeMillis();
        zkw.findWalk(weight,servers,clients_init,count,num);
        System.out.println(zkw.ans);
